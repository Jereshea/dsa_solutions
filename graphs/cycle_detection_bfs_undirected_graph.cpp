...
bool bfs_traversal(int i, unordered_map<int, list<int>> &adjacency_matrix,
  ...
  while (!q.empty()) {
    ...
    for (auto &m : adjacency_matrix[frontNode]) {
      if (visited[m] && (m != parent[frontNode])) {
...
bool cycle_detection_bfs(int vertex, vector<vector<int>> &edges) {
  ...
  for (int i = 0; i < vertex; i++) {
    if (!visited[i]) {
      if (bfs_traversal(i, adjacency_matrix, visited, parent)) {
        return true;
      }
    }
  }
...
int main() {
  ...
  if (cycle_detection_bfs(vertex, edges)) {
