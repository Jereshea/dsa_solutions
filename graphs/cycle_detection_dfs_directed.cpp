...
If we have a graph, it is a directed graph. So when we maintain a visited[]
...
bool dfs_traversal(int i, unordered_map<int, list<int>> &adjacency_matrix,
  ...
  for (auto &m : adjacency_matrix[i]) {
    if (visited[m] && (dfs_visited[m])) {
      return true;
    } else if (!visited[m]) {
      if (dfs_traversal(m, adjacency_matrix, visited, dfs_visited)) {
        return true;
      }
    }
  }
...
bool cycle_detection_dfs(int vertex, vector<vector<int>> &edges) {
  ...
  for (int i = 0; i < vertex; i++) {
    if (!visited[i]) {
      if (dfs_traversal(i, adjacency_matrix, visited, dfs_visited)) {
        return true;
      }
    }
  }
