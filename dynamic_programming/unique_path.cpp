...
example in a given point MATRIX[i] [j], you can move to either MATRIX[i + 1][j]
or MATRIX[i][j + 1].

Input Format:
The first line of input contains an integer 'T' representing the number of the
test case.

The first and the only line of each test case contains two space-separated
integers ‘M’ and ‘N’, denoting the number of rows and number of columns of the
matrix respectively. Output Format: For every test case, return a single
integer, which is the total number of unique paths for traveling from top-left
to bottom-right cells of the matrix.
...
which is both the starting and ending point. Hence the output is 1. Sample Input
2:
2
3 2
1 6
...
In test case 1, we are given a 3 x 2 matrix, to move from matrix[0][0] to
matrix[2][1] we have the following possible paths.
...
int find_unique_path(int m, int n, vector<vector<int>> &unique_path) {
  if (m == 0 || n == 0) {
    return 1;
  }
  ...
  int left = find_unique_path(m, n - 1,
                              unique_path); // backtracing so,right will be n-1
  int up = find_unique_path(m - 1, n,
                            unique_path); // backtracing so,down will be m-1
  unique_path[m][n] = left + up;
  return unique_path[m][n];
}
int main() {
  ...
  int m = 2;
  int n = 2;
  vector<vector<int>> unique_path(m, vector<int>(n, -1));
  cout << find_unique_path(m - 1, n - 1, unique_path);
